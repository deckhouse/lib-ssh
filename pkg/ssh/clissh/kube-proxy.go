...
func NewKubeProxy(sett settings.Settings, sess *session.Session) *KubeProxy {
	return &KubeProxy{
		settings:                sett,
		Session:                 sess,
		port:                    "0",
...
func (k *KubeProxy) Start(useLocalPort int) (port string, err error) {
	...
	logger.DebugF("Kube-proxy start id=[%d]; port:%d\n", startID, useLocalPort)

	success := false
	defer func() {
		k.stop = false
		if !success {
			logger.DebugF("[%d] Kube-proxy was not started. Try to clear all\n", startID)
			k.Stop(startID)
		}
		logger.DebugF("[%d] Kube-proxy starting was finished\n", startID)
	}()

	proxyCommandErrorCh := make(chan error, 1)
	proxy, port, err := k.runKubeProxy(proxyCommandErrorCh, startID)
	if err != nil {
		logger.DebugF("[%d] Got error from runKubeProxy func: %v\n", startID, err)
		return "", err
	}

	logger.DebugF("[%d] Proxy was started successfully\n", startID)
	...
	if lastError != nil {
		logger.DebugF("[%d] Got error from upTunnel func: %v\n", startID, err)
...
func (k *KubeProxy) Stop(startID int) {
	...
	if k.proxy != nil {
		logger.DebugF("[%d] Stop proxy command\n", startID)
		k.proxy.Stop()
		logger.DebugF("[%d] Proxy command stopped\n", startID)
		k.proxy = nil
		k.port = "0"
	}
	if k.tunnel != nil {
		...
		k.tunnel.Stop()
		logger.DebugF("[%d] Tunnel stopped\n", startID)
		k.tunnel = nil
	}
	k.stop = true
}
...
func (k *KubeProxy) tryToRestartFully(startID int) {
	logger := k.settings.Logger()
	logger.DebugF("[%d] Try restart kubeproxy fully\n", startID)
	for {
		k.Stop(startID)
		...
		if err == nil {
			k.stop = false
			logger.DebugF("[%d] Proxy was restarted successfully\n", startID)
			return
		}
		...
		// need warn for human
		logger.WarnF(
			"Proxy was not restarted: %v. Sleep %d seconds before next attempt.\n",
			err,
			sleepTimeout,
		)
		...
		logger.DebugF("[%d] New host selected %v\n", startID, k.Session.Host())
	}
}
...
func (k *KubeProxy) proxyCMD(startID int) *Command {
	...
	command := fmt.Sprintf("PATH=$PATH:%s/; %s", k.settings.NodeBinPath(), kubectlProxy)

	k.settings.Logger().DebugF("[%d] Proxy command for start: %s\n", startID, command)

	cmd := NewCommand(k.settings, k.Session, command)
...
func (k *KubeProxy) healthMonitor(
	...
	defer logger.DebugF("[%d] Kubeproxy health monitor stopped\n", startID)
	logger.DebugF("[%d] Kubeproxy health monitor started\n", startID)

	for {
		logger.DebugF("[%d] Kubeproxy Monitor step\n", startID)
		select {
		case err := <-proxyErrorCh:
			logger.DebugF("[%d] Proxy failed with error %v\n", startID, err)
		...
		case err := <-tunnelErrorCh:
			logger.DebugF("[%d] Tunnel failed %v. Stopping previous tunnel\n", startID, err)
			// we need fully stop tunnel because
			k.tunnel.Stop()

			logger.DebugF("[%d] Tunnel stopped before restart. Starting new tunnel...\n", startID)
...
func (k *KubeProxy) upTunnel(
	...
	for {
		...
		tun = NewTunnel(k.settings, k.Session, "L", tunnelAddress)
		err := tun.Up()
		if err != nil {
			logger.DebugF("[%d] Start tunnel was failed. Cleaning...\n", startID)
			tun.Stop()
			...
			if retries >= maxRetries {
				logger.DebugF("[%d] Last iteration finished\n", startID)
				tun = nil
				break
			}
		} else {
			logger.DebugF("[%d] Tunnel was started. Starting health monitor\n", startID)
...
func (k *KubeProxy) runKubeProxy(
	...
	proxy.WithStdoutHandler(func(line string) {
		...
		if len(m) == 2 && m[1] != "" {
			port = m[1]
			logger.DebugF("Got proxy port = %s on host %s\n", port, k.Session.Host())
			portReady <- struct{}{}
		}
	})
	...
	proxy.OnCommandStart(func() {
		logger.DebugF("[%d] Command started\n", startID)
		onStart <- struct{}{}
	})

	proxy.WithWaitHandler(func(err error) {
		logger.DebugF("[%d] Wait error: %v\n", startID, err)
		waitCh <- err
	})
...
Status: %w`
	...
	case <-t.C:
		logger.DebugF("[%d] Starting proxy command timeout\n", startID)
		return nil, "", fmt.Errorf("timeout waiting for api proxy port")
	case <-portReady:
		if port == "" {
			logger.DebugF("[%d] Starting proxy command: empty port\n", startID)
			return nil, "", fmt.Errorf("got empty port from kubectl proxy")
		}
	}

	logger.DebugF("[%d] Proxy process started with port: %s\n", startID, port)
