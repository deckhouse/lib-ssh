...
func (p *SSHProvider) Client() (connection.SSHClient, error) {
...
func (p *SSHProvider) SwitchClient(_ context.Context, sess *session.Session, privateKeys []session.AgentPrivateKey, _ connection.SSHClient) (connection.SSHClient, error) {
...
func errorCommand(name, errStr string) connection.Command {
...
func (c *Client) Command(name string, arg ...string) connection.Command {
	...
	host := c.SessionSettings.Host()
	providers, err := c.commandProviders.get(host)
	if err != nil {
		return errorCommand(name, err.Error())
	}

	bastion := getBastion(c.SessionSettings)
...
func errorFile(errStr string) connection.File {
...
func errorScript(path, errStr string) connection.Script {
...
func (c *Client) Check() connection.Check {
	...
	return utils.NewCheck(f, c.SessionSettings, c.Settings)
}
...
func (c *Client) Loop(fn connection.SSHLoopHandler) error {
	...
	resetSession := func() {
		c.SessionSettings = c.SessionSettings.Copy()
		c.SessionSettings.ChoiceNewHost()
	}
	defer resetSession()
	resetSession()

	for range c.SessionSettings.AvailableHosts() {
		err = fn(c)
		if err != nil {
			return err
		}
